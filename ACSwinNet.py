...
import numpy as np
import config_Rigid as configs
...
def window_partition(x, window_size):
    ...
    Returns:
        ...
        pk: 第二次partion，不知道embedding来过吗？试试看。
    """
    B, H, W, L, C = x.shape
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        x = nnf.pad(x,(0,0,0,pad_h,0,pad_b,0,pad_r))
        ...
        if pad_r > 0 or pad_b > 0 or pad_h > 0: #把pad的数据去掉
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, T):
        ...
        # calculate attention mask for SW-MSA 保证是window_size的整数倍，这是对于mask而言：
        ...
        h_slices = (slice(0, -self.window_size[0]), #slice 切片：
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if W % self.patch_size[1] != 0:
            padding_W = self.patch_size[1] - W % self.patch_size[1]
            if padding_W % 2 ==0: #说明是偶数
                x = nnf.pad(x, (0,0, padding_W/2,padding_W/2))
            else: #说明是奇数
                x = nnf.pad(x, (0,0, int(padding_W / 2)+1,int( padding_W / 2)))
        if H % self.patch_size[0] != 0:
            padding_H = self.patch_size[0] - H % self.patch_size[0]
            if padding_H % 2 == 0:  # 说明是偶数
                x = nnf.pad(x, (0, 0,0,0, padding_H / 2, padding_H / 2))
            else:  # 说明是奇数
                x = nnf.pad(x, (0, 0,0,0, int(padding_H / 2) + 1, int(padding_H / 2)))

        if T % self.patch_size[0] != 0:
            padding_T = self.patch_size[0] - T % self.patch_size[0]
            if padding_T % 2 == 0:  # 说明是偶数
                x = nnf.pad(x, (padding_T / 2, padding_T / 2))
            else:  # 说明是奇数
                x = nnf.pad(x, (int(padding_T / 2) + 1, int(padding_T / 2)))

        x_n = x.detach().cpu().numpy()
        x = self.proj(x)  # B C Wh Ww Wt #实际用的是4x4的卷积进行embedding
        x_n = x.detach().cpu().numpy()
...
class ACSwinNetRigid(nn.Module):
    def __init__(self, config):
        super(ACSwinNetRigid, self).__init__()
        if_convskip = config.if_convskipR
        ...
        self.conv = nn.Conv3d(48,12,kernel_size=3,stride=1)
    ...
    def forward(self, x,x_seg):
        source = x[:, 0:1, :, :,:] #取了x,y拼接体中的x。
        source_seg =x_seg #取出来的是seg
        out = self.transformer(x) #
        #outn = out[-1].detach().cpu().numpy()
        x = self.bn1(out[-1])
        x = self.conv(x)
        ...
        out,out_seg, mat, inv_mat = self.Rigid_trans(source,source_seg, rigid, trans)
        return out,out_seg, mat, inv_mat

class RigidTransformer(nn.Module):
    """
    3-D Rigid Transformer
    """
    ...
    def forward(self, src,src_seg, Rigid, translate):
        ...
        mat = torch.bmm(rot_mat_z, torch.matmul(rot_mat_y, rot_mat_x))
        inv_mat = torch.inverse(mat)
        mat = torch.cat([mat, trans], dim=-1) #最后一维上进行拼接。
        inv_trans = torch.bmm(-inv_mat, trans)
        inv_mat = torch.cat([inv_mat, inv_trans], dim=-1)

        grid = nnf.Rigid_grid(mat, [src.shape[0], 3, src.shape[2], src.shape[3], src.shape[4]], align_corners=True)
        grid_seg = nnf.Rigid_grid(mat, [src_seg.shape[0], 3, src_seg.shape[2], src_seg.shape[3], src_seg.shape[4]], align_corners=True)
        out = nnf.grid_sample(src, grid, align_corners=True, mode=self.mode)
        out_seg = nnf.grid_sample(src_seg, grid_seg, align_corners=True, mode='nearest')
        # out_seg_n = out_seg.cpu().detach().numpy()

        # out_n =  out.cpu().detach().numpy()
        return out,out_seg, mat, inv_mat



class ApplyRigid(nn.Module):
    """
    3-D Rigid Transformer
    """
    def __init__(self, mode='bilinear'):
...
CONFIGS = {
    'ACSwinNet-Rigid': configs.get_3DACSwinNetRigid_config(),
}
